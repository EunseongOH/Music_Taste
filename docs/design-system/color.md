...
| 곡 고르기 나가기(2단계 마법사 → 시트 하나), "확인해 주세요" 알림(→ 토스트) | 적용 |
| 아티스트 고르기 나가기, 한 아티스트 확인, `BackButton` 로그인 전 경고, 취향표 로그인 전 나가기 | 적용 (시트) |
| 취향표 덮어쓰기 팝업의 **취소** 버튼이 빨강이던 것 | 적용 (보조 버튼 색으로. 취소는 잃는 행동이 아니다) |
| 홈 진행 중인 월드컵 확인 — 이어서 하기 · 새로 시작(`dangerButton`) | 적용 (시트) |
...
팝업과 시트 중 무엇을 쓸지, 나가기 흐름의 선택지는 `dialogs.md` 에 있다. 남은 것을 옮길 때는 한 화면씩, 문구도 `ux-writing.md` 에 맞춘다(예: "저장하지 않고 나가기"는 그대로, "정말 삭제하시겠습니까?" → "취향표를 삭제할까요?").
