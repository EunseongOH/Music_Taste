...
| 내 취향 스페이스 · 우리의 취향 아카이브 | 보조 | 화면은 그대로. **믹스 매치 취향표 27건은 목록에서 가린다**(2026-09-21 사용자 결정, 부록 A-6). 행은 DB 에 그대로 있다 |

접는 것은 **만드는 길**과 **목록 노출**이지 **데이터**가 아니다. 기존 믹스 매치 취향표는 목록(내 취향 스페이스·우리의 취향 아카이브·프로필 창)에서 가리되, 행·뱃지 코드·공유 링크(`/taste/[id]`)는 그대로 둔다. `MIX_MATCH` 를 켜면 27건이 그대로 돌아온다.
...
- `archive/page.tsx`, `explore-taste/page.tsx` 의 "믹스 매치" 뱃지 **코드** — 목록에서는 가려져 지금은 보일 일이 없지만(A-6), 되살렸을 때 27건이 그렇게 표시돼야 하므로 지우지 않는다.
...
4. 스토어 스크린샷 재촬영 — 완료(2026-09-23, `toss/store/capture-store.mjs`. 옛 `capture-screenshots.mjs`·`fixture-mix.json` 은 지웠다).
...
6. **같이 소트하기 초대 화면의 "섞인 방"을 정식화한다.** 믹스 매치가 살아나면 한 방에 여러
   ...
   내렸다) 최소 대체만 두었다 — 제목을 쓰는 문구("'{방 제목}' 소트에 초대받았어요!")와
   첫 곡 재킷 배경. 되살릴 때 이 규칙을 정식 디자인으로 올린다(2026-09-22 사용자 지시).
...
### A-6. 결과(취향표) 격리 범위 — 확정 (2026-09-21 사용자 결정)

처음 계획은 "이미 만들어진 믹스 매치 취향표는 그대로 보인다"였다. 사용자가 **목록에서도 가리기로** 정했다. 진입점이 없는 모드의 결과물이 아카이브의 3분의 2(공개 41건 중 27건)를 차지하면, 새 리스너가 만들 수 없는 것을 먼저 보게 된다.

| 가리는 곳 | 방법 |
|---|---|
| 우리의 취향 아카이브 (`archive/page.tsx`) | 목록 쿼리에 `.in("is_single_artist", VISIBLE_MODES)` |
| 내 취향 스페이스 (`explore-taste/page.tsx`) — 내 취향표, 다른 리스너 취향표(취향 메이트 계산 포함) | 같음 |
| 프로필 창 완료 목록 (`ProfileModal.tsx`) | 같음 |
| 같이 소트하기 "이미 한 소트에서 가져오기" (`together/new/page.tsx`) | 같음 |

`VISIBLE_MODES` 는 `src/config/modes.ts` 에서 `MIX_MATCH` 로부터 계산된다(`[true]` 또는 `[true, false]`). **플래그 한 줄만 바꾸면 결과·초안·진입점이 함께 돌아온다.** 되살리는 절차(§10)에 추가할 일은 없다.

**가리지 않는 것**: 주소로 직접 여는 공유 취향표(`/taste/[id]`). 이미 나간 링크를 끊지 않는다. 그 화면의 "나도 해 보기"는 이미 한 아티스트 모드로 보낸다.

**영향**: 아카이브 공개 취향표가 41건 → 14건으로 준다. 믹스 매치로만 저장한 리스너는 내 취향 스페이스가 비어 보인다(빈 상태 문구가 뜬다). 데이터는 남아 있으므로 되살리면 그대로 보인다. §7.4 페이지네이션은 이 필터 **뒤에** 붙인다(서버에서 먼저 걸러야 20건이 꽉 찬다).
