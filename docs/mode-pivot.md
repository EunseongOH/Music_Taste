...
즉 이번 429 는 "운영자가 테스트해서 생긴 예외"가 아니라 **실이용자 1명이 그대로 재현할 수 있는 구조적 한계**다. §7 의 대책이 여기에 맞춰 바뀐다 — 핵심은 상한이 아니라 **둘러보기를 싸게 만드는 것**이다.
...
원칙이 하나다. **고르기 전에는 싸게, 고른 뒤에만 비싸게.** 지금은 반대로 되어 있다 — 아티스트를 눌러보기만 해도 전곡을 받아온다(§2.1).

우선순위대로 적는다. 7.0 과 7.1 이 이번 429 의 직접 대책이다.

### 7.0 개발·검사가 운영 쿼터를 태우지 않게 한다 (가장 먼저, 가장 쌈)

이번 차단은 운영자가 아티스트 여러 명을 열어보다 난 것이다. 로컬 개발 서버도, Playwright 검사도 **운영과 똑같은 Spotify 앱·똑같은 쿼터**를 쓴다. 이용자 없는 날에도 문이 닫힐 수 있다.

- `.env.local` 스위치 `SPOTIFY_CACHE_ONLY=1` 을 만든다. 켜져 있으면 `spotifyFetch` 는 **캐시만 보고, 미스면 빈 결과를 돌려준다**(예외를 던지지 않는다 — 화면이 깨지면 안 된다).
- 로컬 개발·검사 기본값을 켜짐으로 둔다. 새 아티스트를 일부러 담아야 할 때만 끈다.
- 화면 확인은 이미 캐시가 찬 아티스트(현재 전곡 20명)로 한다.

한 줄 요약: **개발하면서 태우지 않는다.** 비용은 이용자에게만 쓴다.

### 7.1 둘러보기를 싸게 만든다 (구조적 대책)

`tracks/page.tsx:584-684` 의 `loadRemainingPagesInBackground` 는 아티스트를 고른 순간 **앨범 전 페이지 + 전 앨범의 곡 목록**을 800ms 간격으로 끝까지 받는다. 이용자가 "이 아티스트로 할까" 하고 세 명만 열어봐도 90콜이다.

바꿀 것 — 두 단계로 나눈다.

| 단계 | 지금 | 바꿀 것 | 콜 |
|---|---|---|---|
| 아티스트를 열었을 때 | 전 앨범 + 전곡 자동 | **앨범 목록만**(+ 최신 앨범 2~3장의 곡) | 3~6콜 |
| "이 아티스트로 소트 시작"을 눌렀을 때 | — | 나머지 앨범의 곡을 백그라운드로 | 26콜 |

둘러보기 비용이 **29콜 → 3~6콜, 약 1/5**이 된다. 이미 캐시가 찬 아티스트는 어느 쪽이든 **0콜**이다.

'시작' 버튼을 새로 만들 필요는 없다 — 전곡 모드에는 이미 아티스트 확정 시트(`explore/page.tsx:1160-1210` `pendingSingleArtist`)가 있다. 그 확정을 프리페치의 방아쇠로 쓴다.
...
⚠️ **상한만 먼저 넣으면 안 된다.** 7.1 없이 상한 60 을 걸면 아티스트 두 명 둘러본 이용자 한 사람이 그날 몫을 다 쓰고, 그 뒤에 온 사람은 아무것도 못 한다. **7.1 → 7.2 순서**로 간다.

**선행 확인**: `spotifyFetch` 가 게이트 false 를 받았을 때의 동작이 "캐시에 있는 것만 보여주고 조용히 멈춘다"인지 확인한다. 아니면 그렇게 고친다. 화면에는 "지금은 이 아티스트의 일부만 준비돼 있어요"로 알린다.

### 7.3 캐시가 찬 아티스트를 먼저 보여준다

`/explore?mode=single` 의 기본 목록을 **`together_artist_catalog` 뷰**(`coverage` = 곡까지 받아 둔 앨범 ÷ Spotify 가 말한 앨범 수) 순으로 정렬하고, 같이 소트하기 만들기 화면과 **같은 API·같은 표시**("전곡 있어요")를 쓴다.

효과가 세 겹이다 — 둘러보기가 0콜이 되고(비용), 고른 순간 바로 곡이 뜨고(속도), 곡이 다 있는 아티스트로 자연히 유도된다(품질). 캐시에 없는 아티스트도 검색으로 계속 갈 수 있게 둔다 — 적재 경로를 막지는 않는다.

### 7.4 Supabase 읽기: 페이지네이션 (확정) · 선호 아티스트 (후순위)

**지금 할 것 — 페이지네이션.** `explore-taste/page.tsx:212-232` 와 `archive/page.tsx:304-307` 이 `tournament_results` 를 `select("*")` + limit 없이 읽는다. 행마다 `ranking` jsonb(곡 전체: 제목·아티스트·커버 URL)가 붙어 있어 결과가 쌓이는 만큼 진입 비용이 선형으로 커진다. 목록에 필요한 열만 고르고 20건씩 끊어 받는다(더 보기 / 무한스크롤).

**나중에 — 선호 아티스트.** 이용자가 늘면 "무엇을 20건 보여줄 것인가"가 문제가 된다. 그때는 이용자가 등록해 둔 선호 아티스트로 **그 아티스트를 소트한 다른 리스너의 취향표를 먼저** 보여준다. 페이지네이션과 같은 자리를 쓰므로 정렬 기준만 바뀐다 — 지금 페이지네이션을 넣어 두면 그때 얹기만 하면 된다. **이 전환의 범위 밖이고 후순위다.**

`spotify.ts` 의 캐시 조회 4곳(`:399, :628, :710, :993`)도 `select('*')` 인데, 이건 서버↔DB 라 사용자 체감과 무관하다. 손대지 않는다.
...
| **P5** | 둘러보기 비용 낮추기 (§7.1) + 캐시된 아티스트 우선 노출 (§7.3) | 아티스트를 열어도 앨범 목록만 받는다. 캐시된 아티스트는 콜 0 |
...
- **P5 → P6 순서를 바꾸지 않는다.** 상한을 먼저 걸면 아티스트 두 명 둘러본 이용자 한 명이 그날 몫을 다 쓴다.
- P7 은 독립적이라 언제 해도 된다.
...
| 위험 | 대비 |
|---|---|
| **쿼터가 다시 터진다** — 원인 엔드포인트는 남는 모드 것이고, 이용자가 아티스트를 둘러보기만 해도 소모된다(§2.1) | P0(개발 트래픽 차단) → P5(둘러보기 비용) → P6(상한) 순서. 상한만 먼저 걸면 더 나빠진다 |
...
| 3 | 9/17 429 의 원인은 **운영자가 전곡 모드에서 아티스트 여러 명의 트랙리스트를 열어본 것** | **확인됨**. 대책은 §7.0·§7.1 |
| 4 | 대역폭: **페이지네이션을 넣는다**. 선호 아티스트 기반 정렬은 **후순위** | **확정** 2026-09-21 — §7.4 |
| 5 | 일일 상한 숫자(albums 60 · tracks 300 · search 120) | §7.1 적용 **후에** 넣는다. 순서를 바꾸면 이용자 한 명이 하루를 다 쓴다 |
...
- §7.1 **적용 전**: 이용자 1명이 아티스트 3명만 둘러봐도 87콜 → 상한 60 이면 첫 사람이 그날을 다 쓴다.
- §7.1 **적용 후**: 둘러보기 3~6콜, 실제로 소트를 시작한 아티스트만 29콜 → 상한 300(곡 목록) 안에서 **하루 새 아티스트 9~10명**, 둘러보기는 사실상 무제한.
