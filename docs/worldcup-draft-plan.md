...
### 5-1. 마이그레이션 `supabase/migrations/20260921000100_worldcup_draft_v2.sql`
...
1. 마이그레이션 작성 → 새벽에 운영 적용 → 즉시 develop 배포 (유니크 인덱스 때문에 붙여서).
2. `worldcupDb.ts`: `toCompact`/`hydrateDraft`/`draftExpiresAt`/`saveWorldcupDraft`/모드 필수화/`picks` + 자체 검사.
3. `worldcup/page.tsx`: picks 누적, 자동저장 페이로드 교체, 모달, popstate 가드, `onAppExit`.
4. `platform.ts` / `platform.toss.ts`: `onAppExit`.
5. 홈·ProfileModal: progressObj 블록 삭제, 남은 시간 표시, 만료 제외.
6. tracks·explore: 배경 저장 가드.
7. taste: `picks` 전달.
8. 9장 검증 → `develop` 푸시.
