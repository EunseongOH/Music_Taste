...
1. **만들기** — 곡 세트만 정하면 된다. 소트 기록이 없어도 된다. 세 가지에서 가져온다.
   - **아티스트에서 고르기** — 아티스트를 찾으면 전곡이 들어온다. 전곡 그대로 하거나 뺄 곡을 끈다.
...
| `/together/new` | 아티스트 찾기 또는 곡 출처 고르기(지금 고른 곡 · 내 취향표) → 곡 켜고 끄기 → 만들기 → 코드·링크 보내기 |
| `/api/together/catalog` | 캐시된 아티스트 찾기(`?q=`)·아티스트 전곡(`?artistId=`). 서버에서 캐시 테이블만 읽는다 |
