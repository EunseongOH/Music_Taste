...
const PAGES = [
  { n: 1, copy: ['좋아하는 곡 중에서도,', '더 마음이 가는 곡을 찾는 곳'], route: '/', logo: true, wait: 3000 },
  {
    n: 2,
  ...
  { n: 4, copy: ['같은 곡을 친구와 각자 소트하고,', '취향이 얼마나 닮았는지 봐요'], route: `/together/${process.env.INVITE_CODE ?? '9vtwkaq'}`, wait: 6000 },
  { n: 5, copy: ['내 취향표는 한곳에 모여요.', '취향이 닮은 리스너도 만나요'], route: '/explore-taste', auth: true, wait: 6000 },
];
...
async function compose(helper, shot, page) {
  await helper.setContent(`
    <style>
      ...
      img.logo{position:absolute;right:44px;top:70px;width:52px;height:52px;border-radius:14px;
         box-shadow:0 4px 12px rgba(${TONE.inkRgb},.16)}
    </style>
    <h1>${page.copy[0]}<br/>${page.copy[1]}</h1>
    <img class="shot" src="data:image/png;base64,${shot.toString('base64')}"/>
    ${page.logo ? `<img class="logo" src="data:image/png;base64,${LOGO}"/>` : ''}
  `);
...
for (const p of PAGES) {
  ...
  const file = join(OUT, `store-${p.n}.png`);
  (await import('node:fs')).writeFileSync(file, out);
  console.log(`  ${p.n}  ${p.route}  body=${bg}  → store-${p.n}.png`);
  await ctx.close();
}
await browser.close();
console.log(`\n저장 위치: ${OUT}`);
